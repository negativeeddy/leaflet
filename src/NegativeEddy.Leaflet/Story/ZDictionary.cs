...
namespace NegativeEddy.Leaflet.Story
{
    ...
    public class ZDictionary
    {
        ...
        public ZDictionary(ReadOnlyMemory<byte> bytes, int address)
        {
        ...
        private void Load()
        {
            ...
            byte numberOfSeparators = span[currentAddress];
            currentAddress++;

            LoadWordSeparators(span, numberOfSeparators, currentAddress);
            currentAddress += numberOfSeparators;

            _entryLength = span[currentAddress];
            currentAddress++;
            ...
            var entryBytes = span.Slice(_entryBaseAddress, _entryLength * _entryCount);
            LoadEntries(entryBytes, _entryCount, _entryLength);
        }

        private void LoadEntries(ReadOnlySpan<byte> bytes, int count, int length)
        {
        ...
        private void LoadWordSeparators(ReadOnlySpan<byte> data, int count, int address)
        {
